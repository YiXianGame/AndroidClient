...
public class AttributeAdapt extends RecyclerView.Adapter<AttributeAdapt.ViewHolder>{
    ...
    @NonNull
    @Override
    public AttributeAdapt.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        //用来创建ViewHolder实例，再将加载好的布局传入构造函数，最后返回ViewHolder实例
        View view= LayoutInflater.from(parent.getContext()).inflate(R.layout.battle_repository_attribute_item,parent,false);
        AttributeAdapt.ViewHolder holder = new AttributeAdapt.ViewHolder(view);
        return holder;
    }
